...
namespace Tests
{
   public class ConfigReloadingProxyReferenceModelTests
   {
      ...
      public void TypeSpecifiedExplicitly()
      {
         ...
         var initialFoo = (Foo)proxyFoo.Object;
         ...
         var changedFoo = (Foo)proxyFoo.Object;
      ...
      public void WhenReloadOnChangeIsChangedToFalseObjectIsNotReloaded()
      {
         ...
         using (var proxy = new ProxyFoo(config.GetSection("foo"), new DefaultTypes(), new ValueConverters(), null!, null!))
         {
            var foo = (IFoo)proxy;
            var proxyFoo = (ConfigReloadingProxy<IFoo>)foo;
            var initialFoo = (Foo)proxyFoo.Object;
